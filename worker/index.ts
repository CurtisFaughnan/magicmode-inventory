...
async function handleApi(request:Request,env:Env,url:URL){await initialize(env.DB);if(url.pathname==="/api/items"&&request.method==="GET"){const r=await env.DB.prepare("SELECT id,part_number partNumber,name,description,qty_needed qtyNeeded,qty_on_hand qtyOnHand,unit_cost unitCost,unit,supplier,lead_time leadTime,bom_level bomLevel,notes FROM items ORDER BY name").all();return json(r.results);}if(url.pathname==="/api/transactions"&&request.method==="GET"){const r=await env.DB.prepare("SELECT t.id,t.item_id itemId,i.name itemName,t.kind,t.quantity,t.reference,t.note,t.created_at createdAt FROM transactions t JOIN items i ON i.id=t.item_id ORDER BY t.id DESC LIMIT 200").all();return json(r.results);}if(url.pathname==="/api/transactions"&&request.method==="POST"){const b=await request.json() as {itemId:number;kind:string;quantity:number;reference?:string;note?:string};if(!b.itemId||!["receive","use","adjust"].includes(b.kind)||!Number.isFinite(b.quantity)||b.quantity<0)return json({error:"Invalid transaction"},400);const item=await env.DB.prepare("SELECT qty_on_hand q FROM items WHERE id=?").bind(b.itemId).first<{q:number}>();if(!item)return json({error:"Item not found"},404);const next=b.kind==="receive"?item.q+b.quantity:b.kind==="use"?item.q-b.quantity:b.quantity;if(next<0)return json({error:"Not enough stock"},409);await env.DB.batch([env.DB.prepare("UPDATE items SET qty_on_hand=? WHERE id=?").bind(next,b.itemId),env.DB.prepare("INSERT INTO transactions(item_id,kind,quantity,reference,note,created_at) VALUES(?,?,?,?,?,?)").bind(b.itemId,b.kind,b.quantity,b.reference||"",b.note||"",new Date().toISOString())]);return json({ok:true,qtyOnHand:next},201);}return json({error:"Not found"},404);}
