...
async function initialize(db:D1Database){await db.batch([db.prepare("CREATE TABLE IF NOT EXISTS items (id INTEGER PRIMARY KEY AUTOINCREMENT, part_number TEXT NOT NULL, name TEXT NOT NULL, description TEXT NOT NULL, qty_needed REAL NOT NULL, qty_on_hand REAL NOT NULL, unit_cost REAL, unit TEXT NOT NULL, supplier TEXT NOT NULL, lead_time TEXT NOT NULL, bom_level INTEGER, notes TEXT NOT NULL)"),db.prepare("CREATE TABLE IF NOT EXISTS transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, item_id INTEGER NOT NULL, kind TEXT NOT NULL, quantity REAL NOT NULL, reference TEXT NOT NULL, note TEXT NOT NULL, created_at TEXT NOT NULL)"),db.prepare("CREATE INDEX IF NOT EXISTS transactions_item_idx ON transactions(item_id)"),db.prepare("CREATE TABLE IF NOT EXISTS purchase_links (item_id INTEGER PRIMARY KEY NOT NULL, url TEXT NOT NULL, updated_at TEXT NOT NULL)")]);const c=await db.prepare("SELECT COUNT(*) c FROM items").first<{c:number}>();if(!c?.c){for(const x of seed)await db.prepare("INSERT INTO items(part_number,name,description,qty_needed,qty_on_hand,unit_cost,unit,supplier,lead_time,bom_level,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(...x).run();}}
const json=(data:unknown,status=200)=>new Response(JSON.stringify(data),{status,headers:{"content-type":"application/json"}});
async function handleApi(request:Request,env:Env,url:URL){await initialize(env.DB);
 if(url.pathname==="/api/items"&&request.method==="GET"){const r=await env.DB.prepare("SELECT i.id,i.part_number partNumber,i.name,i.description,i.qty_needed qtyNeeded,i.qty_on_hand qtyOnHand,i.unit_cost unitCost,i.unit,i.supplier,i.lead_time leadTime,i.bom_level bomLevel,i.notes,COALESCE(p.url,'') purchaseUrl FROM items i LEFT JOIN purchase_links p ON p.item_id=i.id ORDER BY i.name").all();return json(r.results);}
 if(url.pathname==="/api/items"&&request.method==="POST"){const b=await request.json() as Record<string,unknown>;const parsed=validateItem(b);if("error" in parsed)return json(parsed,400);const x=parsed.item;const result=await env.DB.prepare("INSERT INTO items(part_number,name,description,qty_needed,qty_on_hand,unit_cost,unit,supplier,lead_time,bom_level,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(x.partNumber,x.name,x.description,x.qtyNeeded,x.qtyOnHand,x.unitCost,x.unit,x.supplier,x.leadTime,x.bomLevel,x.notes).run();const id=Number(result.meta.last_row_id);if(x.purchaseUrl)await env.DB.prepare("INSERT INTO purchase_links(item_id,url,updated_at) VALUES(?,?,?)").bind(id,x.purchaseUrl,new Date().toISOString()).run();return json({ok:true,id},201);}
 const itemMatch=url.pathname.match(/^\/api\/items\/(\d+)$/);if(itemMatch&&request.method==="PUT"){const id=Number(itemMatch[1]);const b=await request.json() as Record<string,unknown>;const parsed=validateItem(b);if("error" in parsed)return json(parsed,400);const x=parsed.item;await env.DB.batch([env.DB.prepare("UPDATE items SET part_number=?,name=?,description=?,qty_needed=?,qty_on_hand=?,unit_cost=?,unit=?,supplier=?,lead_time=?,bom_level=?,notes=? WHERE id=?").bind(x.partNumber,x.name,x.description,x.qtyNeeded,x.qtyOnHand,x.unitCost,x.unit,x.supplier,x.leadTime,x.bomLevel,x.notes,id),x.purchaseUrl?env.DB.prepare("INSERT INTO purchase_links(item_id,url,updated_at) VALUES(?,?,?) ON CONFLICT(item_id) DO UPDATE SET url=excluded.url,updated_at=excluded.updated_at").bind(id,x.purchaseUrl,new Date().toISOString()):env.DB.prepare("DELETE FROM purchase_links WHERE item_id=?").bind(id)]);return json({ok:true});}
 if(itemMatch&&request.method==="DELETE"){const id=Number(itemMatch[1]);await env.DB.batch([env.DB.prepare("DELETE FROM purchase_links WHERE item_id=?").bind(id),env.DB.prepare("DELETE FROM transactions WHERE item_id=?").bind(id),env.DB.prepare("DELETE FROM items WHERE id=?").bind(id)]);return json({ok:true});}
 const linkMatch=url.pathname.match(/^\/api\/items\/(\d+)\/purchase-link$/);if(linkMatch&&request.method==="PUT"){const itemId=Number(linkMatch[1]);const b=await request.json() as {url?:string};const value=(b.url||"").trim();if(value){try{const parsed=new URL(value);if(!["http:","https:"].includes(parsed.protocol))throw new Error();}catch{return json({error:"Enter a valid http or https purchase link."},400);}await env.DB.prepare("INSERT INTO purchase_links(item_id,url,updated_at) VALUES(?,?,?) ON CONFLICT(item_id) DO UPDATE SET url=excluded.url,updated_at=excluded.updated_at").bind(itemId,value,new Date().toISOString()).run();}else await env.DB.prepare("DELETE FROM purchase_links WHERE item_id=?").bind(itemId).run();return json({ok:true,url:value});}
...
function validateItem(b:Record<string,unknown>){const name=String(b.name||"").trim();const qtyNeeded=Number(b.qtyNeeded);const qtyOnHand=Number(b.qtyOnHand);const unit=String(b.unit||"").trim();const unitCost=b.unitCost===null||b.unitCost===""?null:Number(b.unitCost);const bomLevel=b.bomLevel===null||b.bomLevel===""?null:Number(b.bomLevel);const purchaseUrl=String(b.purchaseUrl||"").trim();if(!name||!unit||!Number.isFinite(qtyNeeded)||qtyNeeded<=0||!Number.isFinite(qtyOnHand)||qtyOnHand<0||unitCost!==null&&(!Number.isFinite(unitCost)||unitCost<0)||bomLevel!==null&&!Number.isFinite(bomLevel))return {error:"Enter a name, unit, positive quantity per chair, and valid non-negative values."};if(purchaseUrl){try{const u=new URL(purchaseUrl);if(!["http:","https:"].includes(u.protocol))throw new Error();}catch{return {error:"Enter a valid http or https purchase link."};}}return {item:{partNumber:String(b.partNumber||"").trim(),name,description:String(b.description||"").trim(),qtyNeeded,qtyOnHand,unitCost,unit,supplier:String(b.supplier||"").trim(),leadTime:String(b.leadTime||"").trim(),bomLevel,notes:String(b.notes||"").trim(),purchaseUrl}};}
