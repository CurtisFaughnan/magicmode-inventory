...
export default function ChairCost({items,onClose,onSaved}:{items:Item[];onClose:()=>void;onSaved:()=>Promise<void>}){
  ...
  async function save(item:Item){
    ...
    if(unitCost===item.unitCost)return;
    const reason=prompt("Why was this price changed?");if(!reason?.trim())return;
    setSaving(item.id);setError("");
    const r=await fetch(`/api/items/${item.id}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({...item,unitCost,priceChangeReason:reason.trim()})});
    const data=await r.json();if(!r.ok)setError(data.error||"The price could not be saved.");else await onSaved();setSaving(null);
  }
  if(historyItem)return <div className="overlay"><section className="modal cost-modal"><PriceHistory item={historyItem} onClose={()=>setHistoryItem(null)}/></section></div>;
  return <div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <section className="modal cost-modal">
      ...
      <div className="cost-table"><table><thead><tr><th>Part</th><th>Per chair</th><th>Unit price</th><th>Cost per chair</th><th></th></tr></thead><tbody>{sortedItems.map(item=><tr key={item.id}><td><strong>{item.name}</strong><small>{item.partNumber||"No part number"}</small></td><td>{item.qtyNeeded} {item.unit}</td><td><div className="price-input"><span>$</span><input aria-label={`${item.name} unit price`} min="0" step="0.01" type="number" placeholder="Blank" value={drafts[item.id]??""} onChange={e=>setDrafts({...drafts,[item.id]:e.target.value})}/></div></td><td>{item.unitCost===null?"—":money.format(item.qtyNeeded*item.unitCost)}</td><td><div className="cost-actions"><button className="ghost" onClick={()=>setHistoryItem(item)}>History</button><button className="ghost" disabled={saving===item.id} onClick={()=>save(item)}>{saving===item.id?"Saving…":"Save"}</button></div></td></tr>)}</tbody></table></div>
